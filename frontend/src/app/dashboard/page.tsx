...
const quickActions = [
  ...
  { icon: HiCog6Tooth, label: 'Pengaturan', href: '/profile', color: 'text-zinc-600 bg-zinc-50' },
];
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-zinc-50">
        <div className="text-center">
          ...
          <p className="mt-4 text-zinc-500 text-sm">Memuat dashboard...</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="min-h-screen bg-zinc-50/50 pb-20">
      ...
      <div className="max-w-2xl mx-auto px-4">
        {/* ─── OVERALL PROGRESS ─── */}
        <div className="mt-6 bg-white rounded-2xl p-5 shadow-sm border border-zinc-100">
          <div className="flex items-center justify-between mb-3">
            <h3 className="font-semibold text-zinc-900">Overall Progress</h3>
        ...
        {/* ─── QUICK ACTIONS ─── */}
        <div className="mt-6">
          <h3 className="text-sm font-semibold text-zinc-500 uppercase tracking-wider mb-3 px-1">Quick Actions</h3>
          <div className="grid grid-cols-4 gap-3">
            {quickActions.map((action) => {
              const Icon = action.icon;
              return (
                <Link key={action.label} href={action.href} className="group">
                  <div className="bg-white rounded-2xl p-4 text-center shadow-sm border border-zinc-100 hover:shadow-md hover:border-zinc-200 transition-all duration-200 active:scale-95">
        ...
        {/* ─── MODULE CARDS ─── */}
        <div className="mt-8">
          <h3 className="text-sm font-semibold text-zinc-500 uppercase tracking-wider mb-3 px-1">Life Modules</h3>
          <div className="space-y-3">
            {modules.map((mod) => {
              const Icon = mod.icon;
              return (
                <Link key={mod.href} href={mod.href} className="block group">
                  <div className="bg-white rounded-2xl p-5 shadow-sm border border-zinc-100 hover:shadow-lg hover:border-zinc-200 transition-all duration-200 active:scale-[0.99]">
                    <div className="flex items-center gap-4">
                      <div className="w-14 h-14 rounded-2xl bg-primary-50 flex items-center justify-center flex-shrink-0 group-hover:scale-110 transition-transform duration-200">
                        <Icon className="w-7 h-7 text-primary-600" />
                      </div>
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center justify-between">
                          <h3 className="text-lg font-bold text-zinc-900">{mod.title}</h3>
                          <HiArrowRight className="w-5 h-5 text-zinc-300 group-hover:text-primary-500 group-hover:translate-x-1 transition-all" />
                        </div>
                        ...
                        <div className="mt-3 flex items-center gap-3">
                          ...
                          <span className="text-xs font-semibold text-zinc-400">{mod.progress}%</span>
                        </div>

                        <div className="flex flex-wrap gap-1.5 mt-3">
                          {mod.features.map((f) => (
                            <span key={f} className="px-2.5 py-0.5 rounded-full text-[10px] font-medium bg-primary-50 text-primary-600">
                              {f}
                            </span>
                          ))}
                        </div>
        ...
        <div className="mt-8 mb-6">
          <Link href="/ai-chat" className="block group">
            <div className="bg-gradient-to-br from-primary-600 to-accent-600 text-white rounded-2xl shadow-xl p-5 relative overflow-hidden">
          ...
          <div className="mb-6">
            <div className="bg-white rounded-2xl p-5 shadow-sm border-2 border-dashed border-amber-200 bg-gradient-to-br from-amber-50/50 to-yellow-50/50">
              <div className="flex items-center gap-3 mb-4">
                ...
                <div>
                  <h3 className="font-bold text-zinc-900">Upgrade Premium</h3>
                  <p className="text-xs text-zinc-500 mt-0.5">Buka semua fitur tanpa batas</p>
                </div>
              </div>
